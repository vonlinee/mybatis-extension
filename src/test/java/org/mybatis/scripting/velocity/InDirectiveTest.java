...
class InDirectiveTest {
  ...
  static void setUpClass() {
    ...
    vc.put(SQLScriptSource.MAPPING_COLLECTOR_KEY,
        new ParameterMappingCollector(new ParameterMapping[] {}, new HashMap<>(), new Configuration()));
    StringWriter writer = new StringWriter();
    velocity.evaluate(vc, writer, "WARM", "1+1");
  }

  @Test
  void ensureInClauseHasEmpty() {
    StringWriter w = new StringWriter();
    vc.put("list", Collections.emptyList());
    velocity.evaluate(vc, w, "TEST", "#in($list $id 'id')?#end");
  ...
  void ensureInClauseHasOne() {
    StringWriter w = new StringWriter();
    vc.put("list", Collections.singletonList("?"));
    velocity.evaluate(vc, w, "TEST", "#in($list $id 'id')?#end");
    String result = w.toString();
